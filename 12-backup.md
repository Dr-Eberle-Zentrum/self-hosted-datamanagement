...
- Ein paar Grundlagen zum Thema Backup lernen
...
Bereits in [Kapitel 7](07-installationsvorbereitung-1.Rmd) wurde auf die Notwendigkeit eines Backup eingegangen und die gesamte Festplatte (bzw. SD-Karte) gesichert. Dadurch besteht eine Möglichkeit, jederzeit den Stand zum Zeitpunkt von Kapitel 7 wiederherzustellen. Auch jetzt ist es empfehlenswert, ein Backup des gesamten Systems zu erstellen.
...
Anschließend können Befehle wie auf der Kommandozeile geschrieben werden. Anschließend muss das Skript gespeichert und mit `sudo chmod ug+x` ausführbar gemacht werden.
...
es werden Variablen für die Backupquelle, das Backupziel definiert und die aktuelle Zeit definiert

es werden leere Verzeichnisse im Backupziel erstellte, sofern diese noch nicht vorhanden sind

es werden Links auf die aktuelle Backupversion erstellt

Nextcloud wird mit dem occ-Tool in den maintanace-Modus versetzt

mit rsync werden die Datenverezeichnisse gesichert
...
Ist der manuelle Test erfolgreich verlaufen, kann das Skript mittels Cron automatisch zu bestimmten Zeiten ausgeführt werden. Wie im vorherigen Kapitel zur Nextcloud-Konfiguration erwähnt, ist Cron ein Programm, mit welchem Befehle zu bestimmten Zeiten automatische ausgeführt werden. Diese Befehle werden in der Crontabelle gespeichert. Jeder User des Betriebssystems hat eine eigene Crontabelle. Soll ein Befehl mit Root-Rechten ausgeführt werden, muss der Befehl in die Crontabelle des root-Account eingetragen werden, für Befehle die durch den www-data-User ausgeführt werden sollen in dessen Cron-Tabelle.
...
Die Einträge für jeden Cron-Job (also jeden auszuführenden Befehl) sind wie folgt aufgebaut: Zeitangabe für die Ausführung und auszuführender Befehl. Die Zeitangaben erfolgt dabei in fünf einzelnen Einheiten: 1. Angabe der Minute (0-59), 2. Angabe der Stunde (0-23), 3. Angabe des Tages (1-31), Angabe des Monats (1-12) und Angabe des Wochentages (0-7).
